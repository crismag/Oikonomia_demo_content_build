# Character Generation Queue

This is the roster/status view. Character-specific topics live only in the linked target files.

## Foundation already developed
- [x] Moses — `ot/moses.md` — SUBSTANTIAL
- [x] Joshua — `ot/joshua.md` — SUBSTANTIAL
- [x] David — `ot/david.md` — SUBSTANTIAL

## Old Testament
- [ ] Aaron — `ot/aaron.md` — FOUNDATION EXISTS
- [ ] Miriam — `ot/miriam.md` — FOUNDATION EXISTS
- [ ] Caleb — `ot/caleb.md` — FOUNDATION EXISTS
- [ ] Deborah — `ot/deborah.md` — FOUNDATION EXISTS
- [ ] Gideon — `ot/gideon.md` — FOUNDATION EXISTS
- [ ] Saul — `ot/saul.md` — FOUNDATION EXISTS
- [ ] Solomon — `ot/solomon.md` — FOUNDATION EXISTS
- [ ] Jehoshaphat — `ot/jehoshaphat.md` — FOUNDATION EXISTS
- [ ] Hezekiah — `ot/hezekiah.md` — FOUNDATION EXISTS
- [ ] Josiah — `ot/josiah.md` — FOUNDATION EXISTS
- [x] Esther — `ot/esther.md` — SUBSTANTIAL
- [ ] Mordecai — `ot/mordecai.md` — FOUNDATION EXISTS
- [x] Samuel — `ot/samuel.md` — SUBSTANTIAL
- [x] Elijah — `ot/elijah.md` — SUBSTANTIAL
- [ ] Elisha — `ot/elisha.md` — FOUNDATION EXISTS
- [ ] Isaiah — `ot/isaiah.md` — FOUNDATION EXISTS
- [ ] Jeremiah — `ot/jeremiah.md` — FOUNDATION EXISTS
- [ ] Daniel — `ot/daniel.md` — FOUNDATION EXISTS
- [ ] Ezra — `ot/ezra.md` — FOUNDATION EXISTS
- [x] Nehemiah — `ot/nehemiah.md` — SUBSTANTIAL
- [ ] Jonah — `ot/jonah.md` — FOUNDATION EXISTS

## Gospels / Twelve
- [x] Peter — `gospels/peter.md` — SUBSTANTIAL
- [ ] Andrew — `gospels/andrew.md` — FOUNDATION EXISTS
- [ ] James son of Zebedee — `gospels/james-son-of-zebedee.md` — FOUNDATION EXISTS
- [ ] John — `gospels/john.md` — FOUNDATION EXISTS
- [ ] Philip — `gospels/philip.md` — FOUNDATION EXISTS
- [ ] Nathanael / Bartholomew — `gospels/nathanael-bartholomew.md` — FOUNDATION EXISTS
- [ ] Matthew — `gospels/matthew.md` — FOUNDATION EXISTS
- [ ] Thomas — `gospels/thomas.md` — FOUNDATION EXISTS
- [ ] James son of Alphaeus — `gospels/james-son-of-alphaeus.md` — FOUNDATION EXISTS
- [ ] Thaddaeus / Judas son of James — `gospels/thaddaeus.md` — FOUNDATION EXISTS
- [ ] Simon the Zealot — `gospels/simon-the-zealot.md` — FOUNDATION EXISTS
- [ ] Matthias — `gospels/matthias.md` — FOUNDATION EXISTS
- [ ] Mary Magdalene — `gospels/mary-magdalene.md` — FOUNDATION EXISTS
- [ ] Martha — `gospels/martha.md` — FOUNDATION EXISTS
- [ ] Mary of Bethany — `gospels/mary-of-bethany.md` — FOUNDATION EXISTS

## Acts and Letters
- [x] Paul — `acts-and-letters/paul.md` — SUBSTANTIAL
- [ ] Barnabas — `acts-and-letters/barnabas.md` — FOUNDATION EXISTS
- [ ] Silas — `acts-and-letters/silas.md` — FOUNDATION EXISTS
- [ ] Luke — `acts-and-letters/luke.md` — FOUNDATION EXISTS
- [ ] John Mark — `acts-and-letters/john-mark.md` — FOUNDATION EXISTS
- [ ] Stephen — `acts-and-letters/stephen.md` — FOUNDATION EXISTS
- [ ] Philip the Evangelist — `acts-and-letters/philip-the-evangelist.md` — FOUNDATION EXISTS
- [ ] Lydia — `acts-and-letters/lydia.md` — FOUNDATION EXISTS
- [ ] Priscilla — `acts-and-letters/priscilla.md` — FOUNDATION EXISTS
- [ ] Aquila — `acts-and-letters/aquila.md` — FOUNDATION EXISTS
- [ ] Apollos — `acts-and-letters/apollos.md` — FOUNDATION EXISTS
- [ ] Cornelius — `acts-and-letters/cornelius.md` — FOUNDATION EXISTS
- [ ] Timothy — `acts-and-letters/timothy.md` — FOUNDATION EXISTS
- [ ] Titus — `acts-and-letters/titus.md` — FOUNDATION EXISTS
- [ ] Phoebe — `acts-and-letters/phoebe.md` — FOUNDATION EXISTS
- [ ] Epaphroditus — `acts-and-letters/epaphroditus.md` — FOUNDATION EXISTS
- [ ] Philemon — `acts-and-letters/philemon.md` — FOUNDATION EXISTS
- [ ] Onesimus — `acts-and-letters/onesimus.md` — FOUNDATION EXISTS
- [ ] Tychicus — `acts-and-letters/tychicus.md` — FOUNDATION EXISTS
- [ ] Aristarchus — `acts-and-letters/aristarchus.md` — FOUNDATION EXISTS

## Data Play persona
- [ ] Cris — `data-play/cris.md` — FOUNDATION EXISTS

## Status
Every character now has a generation-ready foundation (profile with Oikonomia identity, notable topics, target checklist; biblical context, relationships, goals and rhythm where Scripture supports them). Record generation has not started except for Moses, Joshua and David.

## Priority suggestion for record generation
Featured personas first (Peter, Paul, Nehemiah, Esther, Priscilla, Timothy, Cris), then the networks that deepen existing Moses/David/Joshua records (Aaron, Miriam, Caleb, Samuel, Saul, Solomon).